...
h1b <- h1b %>%
  left_join(count_city, by = c("city","state")) #join h1b dataset
...
h1b <- h1b %>%
  left_join(count_city_state[c("state","city","applyrank_city_state")], by = c("city","state"))#join h1b dataset
```
...
h1b <- h1b %>%
  left_join(count_employer_city[c("state","city","EMPLOYER_NAME","apply_employer_city","applyrank_employer_city")],
            by = c("state","city","EMPLOYER_NAME"))#join h1b dataset
```
###3.1.4 rank of job
```
...
h1b <- h1b %>%
  left_join(salary_city_state[c("city","state","wagerank_city_state")], 
...
h1b <- h1b %>%
  left_join(salary_employer_city[c("wage_employer_city","EMPLOYER_NAME","wagerank_employer_city","state","city")], 
            by = c("EMPLOYER_NAME","state","city"))#join h1b dataset
```
```{r save}
names(h1b)
saveRDS(h1b,"h1b.rds")
