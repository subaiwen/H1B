...
#replace 
h1b$WORKSITE <- str_replace(h1b1$WORKSITE, ".*#.*,","unknown,")
```
###2.1.2with digit
```{r digit}
#detect
...
count_site <- h1b %>%
  group_by(WORKSITE) %>%
  summarise(apply_city_us = sum(!is.na(CASE_STATUS)|!is.na(WORKSITE))) %>%
  arrange(desc(apply_city_us)) %>%
  mutate( applyrank_city_us = min_rank (desc(apply_city_us)))
saveRDS(count_site,"count_site.rds") #for ggmap

count_city <- count_site %>%
  separate(WORKSITE,c("city","state"),", ",remove = FALSE) %>%
  select(-WORKSITE)
#within state
count_city_state <- h1b %>%
  group_by(state, city) %>%
  summarise(apply_city_state = sum(!is.na(CASE_STATUS))) %>%
  group_by(state) %>%
  mutate( applyrank_city_state = min_rank (desc(apply_city_state)))
...
count_employer_city <- h1b %>%
  group_by(state, city, EMPLOYER_NAME) %>%
  summarise(apply_employer_city = sum(!is.na(CASE_STATUS)|!is.na(EMPLOYER_NAME)|!is.na(city))) %>%
  group_by(state, city) %>%
  mutate( applyrank_employer_city = min_rank (desc(apply_employer_city)))

count_employer_city <- count_city %>% 
  full_join(count_employer_city, by = c("state","city")) %>%
...
salary_site <- h1b_salary %>%
  group_by(WORKSITE) %>%
  summarise(wage_city_us = mean(PREVAILING_WAGE, na.rm = TRUE, trim = 0.005),
            apply_city_us = sum(!is.na(CASE_STATUS)|!is.na(EMPLOYER_NAME))) %>%
  filter(apply_city_us >= 10) %>%
  arrange(desc(wage_city_us)) %>%
  mutate( wagerank_city_us = min_rank (desc(wage_city_us)))
salary_city <- salary_site %>%
  separate(WORKSITE,c("city","state"),", ",remove = FALSE) %>%
  select(-WORKSITE)
#within state
salary_city_state <- h1b_salary %>%
  group_by(state, city) %>%
  summarise(wage_city_state = mean(PREVAILING_WAGE, na.rm = TRUE, trim = 0.1),
            apply_city_us = sum(!is.na(CASE_STATUS)|!is.na(EMPLOYER_NAME))) %>%
  filter(apply_city_us >= 10) %>%
  group_by(state) %>%
